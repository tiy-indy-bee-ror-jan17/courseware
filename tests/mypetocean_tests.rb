...
class MyPetOceanTest < Minitest::Test
  ...
  def setup
    ...
    @assignment = Assignment.create!(course_id: @course.id, name: 'Destroy C-138', percent_of_grade: '89', due_at: '2017-01-18 21:06:59.001', active_at: '2017-01-18 21:06:59.001')
    @pre_class_assignment = Assignment.create!(course_id: @course.id, name: 'Destroy C-139', percent_of_grade: '89', due_at: '2017-01-18 21:06:59.001', active_at: '2017-01-18 21:06:59.001')
    @pre_class_lesson = Lesson.create!(pre_class_assignment_id: @pre_class_assignment.id, name: 'Karl Jaspers')
    @lesson = Lesson.create!(name: 'Miguel de Unamuno')
    ...
    @course_instructor = CourseInstructor.create(instructor_id: @instructor.id)
    @assignment = Assignment.create!(course_id: @course.id, name: 'Find the Kitten and Eat It with Copious Cheddar', percent_of_grade: '100', due_at: '2017-01-18 21:06:59.001', active_at: '2017-01-18 21:06:59.001')
    @assignment_grade = AssignmentGrade.create!(assignment_id: @assignment.id)
  end
  ...
  def affirm(expression, as_class='')
    association = expression.split('.')[1] # pull the word which follows the period (.)
    association = association[0...-1] if association[-1] == 's' # drop the final 's', if any
    association = association.gsub(/_/, ' ').titlecase.gsub(/ /, '') # convert from snake case to camel case
    association = as_class if !as_class.empty? # optional argument for cases where the association name does not match the class
